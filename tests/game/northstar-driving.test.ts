...
for (const drivingModel of ["arcade", "simulation"] as const) {
  test(`${drivingModel} drives the entire Northstar ascent and descent with normal controls`, () => {
    const stream = new CityStream();
    for (const roadId of ["northstar-highway", "silver-run-switchbacks", "spruce-gorge-viaduct"]) {
