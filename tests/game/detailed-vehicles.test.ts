...
test("both detailed vehicles have bounded, nondegenerate polygon geometry and distinct silhouettes", () => {
  ...
  for (const id of ["accord-v6", "crown-cab"] as const) {
    ...
    assert.ok(faces.length > 900 && faces.length <= MAX_VEHICLE_SURFACE_FACES);
    ...
    assert.ok(new Set(model.wheel.flatMap(f => f.corners.map(p => p.x.toFixed(3)))).size > 15, "round wheels have more than box corners");
  }
