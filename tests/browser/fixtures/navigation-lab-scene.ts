...
import { CYAN, defaultCameraBoom, RED, YELLOW } from "../../../game/config";
...
job.pickup = { x: 8, y: -360 }; job.pickupApproach = { x: 0, y: -360 };
const stream = new CityStream();
const canvas = document.createElement("canvas");
canvas.style.cssText = "position:fixed;inset:0;width:100vw;height:100vh";
document.body.append(canvas);
let renderer: Renderer;
function draw(mode: CameraMode, patch: Partial<NavigationSettings> = {}, offRoute = true, objective: "dropoff" | "custom" | "pickup" = "dropoff") {
  game.onboard = objective === "dropoff";
  game.customDestination = objective === "custom" ? { x: 0, y: -360 } : null;
  const settings = normalizeNavigationSettings({ arrowVisibility: "destination", arrowTarget: "destination", arrowSmoothing: "instant",
    routePickups: true, routeStyle: "both", rerouteMode: "locked", ...patch });
  ...
  const color = objective === "custom" ? YELLOW : objective === "pickup" ? CYAN : RED;
  return { columns: boxes.filter(box => box.sz >= 40).length, total: boxes.length, offRoute: navigation.offRoute,
    matchingColor: boxes.every(box => box.color.slice(0, 3).every((channel, index) => channel === color[index])),
