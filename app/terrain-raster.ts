      ...
      for (let x = left; x <= right; x += 1) {
        if (ab >= -1e-5 && bc >= -1e-5 && ca >= -1e-5) {
          if (!ghost && color[3] < 0.99 && depth > this.depths[pixel]) {
            ...
            const retainedAlpha = this.pixels[offset + 3] / 255 * (1 - alpha);
            const outAlpha = alpha + retainedAlpha;
            if (outAlpha > 0) {
              this.pixels[offset] = (r * alpha + this.pixels[offset] * retainedAlpha) / outAlpha;
              this.pixels[offset + 1] = (g * alpha + this.pixels[offset + 1] * retainedAlpha) / outAlpha;
              this.pixels[offset + 2] = (blue * alpha + this.pixels[offset + 2] * retainedAlpha) / outAlpha;
              this.pixels[offset + 3] = outAlpha * 255;
            }
